...
### Changed
- `README.md` — the PhyloTreeUMAP quick start now documents the complete stage-by-stage MGT and MLT workflows, including schema-specific `--locus-file` inputs, the phylogenetic averaging default, sentinel/structural-missing semantics, and standalone HTML rendering.

### Fixed
- `egt phylotreeumap mgt-mlt-umap` — MLT mode crashed with `ValueError: too many values to unpack (expected 2)` because the `--locus-file` was always parsed as the two-column MGT alg_combo_to_ix file, even when it was the ALG RBH file that MLT mode documents. The locus-file format is now detected from its schema (`rbh` header for MLT; validated `('rbh1', 'rbh2')<TAB>index` rows for MGT), including minimal two-column RBH files. In MLT mode the matrix is validated as square with one row per RBH locus, and UMAP coordinates are attached to the RBH metadata instead of the sample dataframe. The `odolGenUMAP` Snakemake rule hit the same crash.
...
- `egt phylotreeumap mlt-matrix` — taxid filtering now selects sparse-matrix rows by dataframe row position instead of external index labels, preventing excluded genomes from leaking into an arithmetic mean (or causing a phylogenetic-weight dimension mismatch) when the sample dataframe has a nonconsecutive index.
...
- `egt phylotreeumap plot-html` — type-ahead autocomplete on the unified search box. As you type, a scrolling dropdown offers matching taxon names and taxids (substring match, case-insensitive); names are drawn from `taxname_list_str`/`taxname` and numbers from `taxid_list_str`/`taxid`. Implemented with a Bokeh `AutocompleteInput` (`search_strategy="includes"`, `restrict=False`, `max_completions=50`).

### Changed
- `egt phylotreeumap plot-html` — reworked viewer layout ("Atlas" design). The three readout panels (Exploration summary, Color legend, Selected rows) are now stacked vertically in a scrollable right column with consistent styling instead of tabbed; the per-plot search boxes are consolidated into one unified search row above the tree and UMAP that filters on taxid number *or* name; a dark theme (`evogeno_dark`) is applied to the figures and widgets; the color legend shows the top 8 clades in two columns until a selection narrows it; the Selected-rows table gains a Color column with luminance-calculated black/white text and alternating dark row stripes; and the whole layout is sized to fit a laptop screen without scrolling. The native Reset tool now resets the full page state (selection, search, ranges).
